...
from training_t import train
from evaluation_t import suffix_prediction_with_temperature_with_stop, evaluate_compliance_with_formula, evaluate_DL_distance, greedy_suffix_prediction_with_stop
...
def main():
    ...
    # Parameters for GPT Transformer
    NHEAD = 4
    NUM_LAYERS = 2
    DROPOUT = 0.2
    ...
    for config in range(N_CONFIGURATIONS):
        ...
        for current_sample_size in range(SAMPLE_SIZE_START_VALUE, SAMPLE_SIZE_START_VALUE + SAMPLE_SIZE_INCREMENT * SAMPLE_SIZE_INCREMENT_ITERATIONS, SAMPLE_SIZE_INCREMENT):
            ...
            for i_form, formula in enumerate(DNF_formulas_infix):
                ...
                # Dataset
                dataset = torch.tensor(np.loadtxt(str(dataset_file_name).replace("FORMULANUMBER", str(i_form))))
                ...
                for exp in range(N_EXPERIMENTS_PER_FORMULA):
                    # Models - Initialize GPT Transformer models
                    ...
                    ########################################################
                    # Experiment with GPT and GPT Greedy
                    ########################################################

                    # Instantiate model
                    model = deepcopy(gpt).to(device)

                    # Training
                    train_acc, test_acc = train(model, train_dataset, test_dataset, MAX_NUM_EPOCHS, EPSILON)
                    
                    # Save the model
                    model_file = pathlib.Path(results_config_folder, f"model_gpt_formula_{i_form}_sample_size_{current_sample_size}_exp_{exp}.pt")
                    ...
                    # GPT Suffix prediction with temperature
                    for current_prefix_len in range(PREFIX_LEN_START_VALUE, PREFIX_LEN_START_VALUE + PREFIX_LEN_INCREMENT * PREFIX_LEN_INCREMENT_ITERATIONS, PREFIX_LEN_INCREMENT):
                        ...
                        print(f"____________________GPT TEMPERATURE PREDICTION formula {i_form} / sample size {current_sample_size} / experiment {exp} / prefix length {current_prefix_len}____________________")
                    ...
                    for current_prefix_len in range(PREFIX_LEN_START_VALUE, PREFIX_LEN_START_VALUE + PREFIX_LEN_INCREMENT * PREFIX_LEN_INCREMENT_ITERATIONS, PREFIX_LEN_INCREMENT):
                        ...
                        print(f"____________________GPT GREEDY PREDICTION formula {i_form} / sample size {current_sample_size} / experiment {exp} / prefix length {current_prefix_len}____________________")
                        print(f"Satisfaction of formula {i_form}:")
                        print("- Train: ", train_sat)
                        print("- Test: ", test_sat)
                        print("DL distance:")
                    ...
                    for current_prefix_len in range(PREFIX_LEN_START_VALUE, PREFIX_LEN_START_VALUE + PREFIX_LEN_INCREMENT * PREFIX_LEN_INCREMENT_ITERATIONS, PREFIX_LEN_INCREMENT):
                        # Instantiate model
                        model = deepcopy(gpt_bk).to(device)
                        ...
                        # Save the model
                        model_file = pathlib.Path(results_config_folder, f"model_gpt_bk_formula_{i_form}_sample_size_{current_sample_size}_exp_{exp}_prefix_len_{current_prefix_len}.pt")
                        torch.save(model.state_dict(), model_file)
                        ...
                        print(f"____________________GPT+BK TEMPERATURE PREDICTION formula {i_form} / sample size {current_sample_size} / experiment {exp} / prefix length {current_prefix_len}____________________")
                        print("Satisfaction:")
                        print("- Train: ", train_sat)
                        print("- Test: ", test_sat)
                        print("DL distance:")
